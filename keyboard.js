...
const langs = TreeNode.fromCsv(`title,id,appeared,rank
A+,aplus,1988,876
B,b,1969,242
C,c,1972,2
D,d,2001,48
...
const theMap = {};
langs.forEach((item) => {
  theMap[item.get("title").toLowerCase().substr(0, 1)] = item;
});
...
class Symbol {
  ...
  pLangs() {
    const text = this._jQueryElement.text().toLowerCase();
    const hit = theMap[text];
    if (!hit) this.hide();
    else {
      const year = hit.get("appeared");
      const rank = hit.get("rank");
      this._jQueryElement.html(
        `<span class="tiny">&nbsp;</span>` +
          hit.get("title") +
          "<span class='tiny tiny2'>#" +
          `${rank}</span>`
      );
      this._state = "age";
    }
